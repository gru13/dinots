...
function bindImportExport() {
  ...
  if (importUiBtn && fileUi) {
    ...
    fileUi.addEventListener('change', async () => {
      ...
      try {
        ...
        // Save via state/db layer by merging then emitting CONFIG_UPDATED.
        // Easiest is to overwrite local CONFIG object by calling bootstrapConfig after saveConfig.
        // But we don't have a direct setter; we can patch by assigning keys.
  ...
  if (resetUiBtn) {
    resetUiBtn.addEventListener('click', async () => {
      // Reload defaults from DEFAULT_CONFIG by re-running bootstrapConfig when no cloud config.
      // If user is signed in, bootstrapConfig merges cloud; otherwise it applies defaults.
  ...
  if (importLogBtn && fileLog) {
    ...
    fileLog.addEventListener('change', async () => {
      ...
      try {
        ...
        // Minimal: overwrite local state fields; persistence handled by state module sync.
